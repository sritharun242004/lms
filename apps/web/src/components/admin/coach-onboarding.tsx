...
export function CoachOnboarding({ approvals }: { approvals: CoachApproval[] }) {
  ...
  async function approveCoach(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast.success("Participant email approved for signup"); event.currentTarget.reset(); window.location.reload();
    } catch (error) { toast.error(error instanceof Error ? error.message : "Unable to approve email"); } finally { setIsSubmitting(false); }
  }
  return <div className="mx-auto flex max-w-4xl flex-col gap-6"><div><p className="mb-2 text-xs font-semibold tracking-[.14em] text-primary uppercase">Access management</p><h1 className="text-3xl font-bold tracking-tight sm:text-4xl">Participant onboarding</h1><p className="mt-2 text-sm text-muted-foreground">Approve a participant email before it can create a CMS account.</p></div><Card><CardHeader><CardTitle>Approve participant email</CardTitle><CardDescription>The participant can use this email once to create an account and will stay signed in for 30 days.</CardDescription></CardHeader><CardContent><form onSubmit={approveCoach} className="flex flex-col gap-3 sm:flex-row"><Input name="email" type="email" required placeholder="participant@example.com" aria-label="Participant email" /><Button type="submit" disabled={isSubmitting} className="shrink-0">{isSubmitting ? <Loader2 className="size-4 animate-spin" /> : <MailPlus className="size-4" />}Approve email</Button></form></CardContent></Card><Card><CardHeader><CardTitle>Approved participant emails</CardTitle><CardDescription>Track pending and completed participant signups.</CardDescription></CardHeader><CardContent className="p-0">{approvals.length === 0 ? <p className="px-6 pb-6 text-sm text-muted-foreground">No participant emails have been approved yet.</p> : <div className="divide-y divide-border">{approvals.map((approval) => <div key={approval.id} className="flex items-center justify-between gap-4 px-6 py-4"><div className="min-w-0"><p className="truncate text-sm font-medium">{approval.email}</p><p className="text-xs text-muted-foreground">Approved {new Date(approval.createdAt).toLocaleDateString()}</p></div><span className={approval.claimedAt ? "text-sm text-emerald-600" : "text-sm text-amber-600"}>{approval.claimedAt ? "Account created" : "Awaiting signup"}</span></div>)}</div>}</CardContent></Card></div>;
}
